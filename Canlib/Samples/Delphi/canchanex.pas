...
uses
  SysUtils, Windows, Messages, Classes, Graphics, Controls, Forms, CAN;
...
begin
  Result := canRead(FCanHnd, id, @msg, dlc, flags, Longint(time));
end;
...
begin
  Result := canReadWait(FCanHnd, id, @msg, dlc, flags, Longint(time), Longint(timeout));
end;
...
function TCanChannelEx.ReadSpecific(id: Longint;
  ...
  var time: Cardinal): integer;
begin
  Result := canReadSpecific(FCanHnd, id, @msg, dlc, flags, Longint(time));
end;
...
begin
  Result := canReadSyncSpecific(FCanHnd, id, Longint(timeout));
end;
...
var stat: canStatus;
begin
  stat := canReadStatus(FCanHnd, Longint(Result));
...
begin
  Result := canReadSpecificSkip(FCanHnd, id, @msg, dlc, flags, Longint(time));
end;
