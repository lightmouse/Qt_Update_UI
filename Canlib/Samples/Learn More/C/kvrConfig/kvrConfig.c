...
void listDevices (void)
{
  ...
  for (i = 0; (status == canOK) && (i < channel_count); i++) {
    ...
    status = canGetChannelData(i, canCHANNELDATA_DEVDESCR_ASCII, name, sizeof(name));
    if (status != canOK) {
      printf("canGetChannelData(canCHANNELDATA_DEVDESCR_ASCII) failed.\n");
      continue;
    }

    status = canGetChannelData(i, canCHANNELDATA_CHAN_NO_ON_CARD, &channel_on_card, sizeof(channel_on_card));
    if (status != canOK) {
